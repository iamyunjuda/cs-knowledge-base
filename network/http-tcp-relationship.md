...
### TCP는 "신뢰성 있는 바이트 전송"을 보장하는 기술이다

TCP는 단순 전송이 아니다. **데이터가 빠짐없이, 순서대로, 정확하게 도착하는 것을 보장**하는 전송 계층 프로토콜이다.

**TCP가 보장하는 것들:**

| 기능 | 설명 | 없으면 어떻게 되나? |
|------|------|---------------------|
| **순서 보장** | 패킷이 뒤죽박죽 도착해도 원래 순서대로 재조립 | 메시지가 뒤섞여서 깨짐 |
| **유실 복구** | 패킷이 중간에 사라지면 자동 재전송 | 데이터 일부가 누락됨 |
| **중복 제거** | 같은 패킷이 두 번 오면 하나 버림 | 같은 데이터가 중복 처리됨 |
| **흐름 제어** | 수신측이 처리할 수 있는 속도에 맞춰 전송 (Window Size) | 수신측이 과부하로 데이터 유실 |
| **혼잡 제어** | 네트워크가 막히면 전송 속도를 줄임 (Slow Start, AIMD) | 네트워크 전체가 마비 |

> 진짜 "단순 전송"은 **UDP**다. UDP는 위의 것들을 아무것도 안 한다. 보내고 끝. 도착 여부, 순서, 중복 전부 신경 안 씀.

다만 **HTTP 입장에서 보면**, TCP는 "바이트를 안전하게 전달해주는 파이프"다. TCP가 내부적으로 뭘 하든 상관없이, HTTP는 TCP에 텍스트를 넣으면 상대방에게 그대로 도착한다고 믿으면 된다. TCP 자체는 전송하는 내용이 HTTP인지 SSH인지 관심 없다.
...
먼저 도메인 이름의 구조를 이해해야 한다. `www.example.com`은 사실 `www.example.com.`이다 (맨 끝에 점=루트):

```
www . example . com . (루트)
 │      │       │      │
 │      │       │      └── 루트 DNS 서버가 관리 (전 세계 13개 클러스터)
 │      │       └────────── TLD DNS 서버가 관리 (.com, .net, .kr 등)
 │      └────────────────── 권한(Authoritative) DNS 서버가 관리
 └───────────────────────── 서브도메인 (권한 DNS가 함께 관리)
```

**재귀 질의 = 로컬 DNS 서버가 "대신 찾아다녀 주는 것"**

사용자는 로컬 DNS 서버에 한 번만 물어보면 된다. 로컬 DNS 서버가 루트 → TLD → 권한 서버를 차례로 찾아다닌다:

```
[브라우저]                [로컬 DNS 서버]           [루트 DNS (.)]
    |                          |                         |
    |-- "shop.example.com     |                         |
    |    의 IP가 뭐야?" ----> |                         |
    |                          |                         |
...
| 서버 | 역할 | 예시 | 특징 |
|------|------|------|------|
| **루트 DNS** | 최상위. TLD 서버 위치를 알려줌 | "`.com`은 `a.gtld-servers.net`에 물어봐" | 전 세계 13개 클러스터 (a~m.root-servers.net), 실제로는 Anycast로 수백 대 |
| **TLD DNS** | 최상위 도메인(.com, .net, .kr 등) 관리 | "`.com` 안의 `example.com`은 `ns1.example.com`이 관리해" | `.com` TLD만 해도 수억 개 도메인 관리 |
| **권한 DNS** | 실제 도메인의 IP를 가진 최종 서버 | "`example.com`의 IP는 `93.184.216.34`야" | 도메인 소유자가 직접 관리 (or 호스팅 업체에 위임) |

**TTL과 캐싱:**

매번 루트부터 찾으면 느리니까, 각 단계의 결과를 **TTL(Time To Live)** 동안 캐싱한다:

```
shop.example.com의 A 레코드: 93.184.216.34, TTL=3600 (1시간)
→ 로컬 DNS 서버가 1시간 동안 이 결과를 캐싱
→ 1시간 내에 같은 질의가 오면 루트부터 다시 찾지 않고 캐시에서 바로 응답
→ TTL 만료 후 다시 질의하면 루트부터 재탐색
```

TTL이 짧으면 IP 변경이 빨리 반영되지만 DNS 서버 부하가 증가하고, TTL이 길면 부하는 줄지만 IP 변경 반영이 느리다.

**재귀 질의 vs 반복 질의:**

- **재귀 질의(Recursive)**: 클라이언트 → 로컬 DNS. "끝까지 찾아서 최종 답을 달라"
- **반복 질의(Iterative)**: 로컬 DNS → 루트/TLD/권한 서버. "모르면 다음에 물어볼 곳을 알려달라"

즉, 브라우저는 재귀 질의로 한 번만 물어보고, 로컬 DNS 서버가 반복 질의로 직접 돌아다니며 답을 찾는 구조다.
