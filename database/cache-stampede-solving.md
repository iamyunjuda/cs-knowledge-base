...
나는 메타데이터 조회 API 서비스를 운영하고 있다. 수백 종류의 메타데이터를 Key별로 Redis에 캐싱해두고, 대부분의 트래픽은 캐시에서 처리된다. 평소에는 DB QPS도 안정적이고, 응답 시간도 50ms 내외로 문제가 없었다.

**시스템 구성:**
- 캐시: Redis 클러스터
- DB: Primary-Replica 구조의 MySQL
- 애플리케이션: 8대의 서버에서 Nginx 로드밸런싱

---

## 1단계: 이상 징후 발견

금요일 오후, Grafana 대시보드를 보다가 이상한 패턴을 발견했다.

```
14:00 — DB QPS 급증 (평소의 50배) → 30초 후 정상화
14:10 — 또 급증 → 30초 후 정상화
14:20 — 또 급증 → 30초 후 정상화
```

**관찰한 현상:**
| 지표 | 평상시 | 문제 발생 시 |
|------|--------|-------------|
| DB QPS | 기준값 유지 | 10~100배 급증 |
| 평균 응답 시간 | ~50ms | 5초 이상 |
| 실패율 | ~0% | 타임아웃 다수 |
| 지속 시간 | - | ~30초 후 자동 복구 |

처음에는 "크론잡이 돌아가나?" 싶었지만, 크론잡 스케줄과는 맞지 않았다. 마케팅 이벤트? 트래픽 자체가 늘진 않았다. 뭔가 **내부적인 원인**이 있었다.
...
MySQL 슬로우 쿼리 로그를 확인했다.

```sql
-- 문제 시간대에 집중된 쿼리들
SELECT * FROM metadata WHERE meta_key = 'popular-banner-config';
SELECT * FROM metadata WHERE meta_key = 'main-page-layout';
SELECT * FROM metadata WHERE meta_key = 'event-popup-settings';
```
...
```
# Redis 캐시 설정 코드를 열어봤다
redis.set(key, value, EX=600)  # TTL 600초 = 10분
```

**TTL이 전부 600초(10분)로 동일했다.** 서버가 배포되거나 재시작되면, 비슷한 시점에 캐시가 세팅되고, 정확히 10분 뒤에 **한꺼번에 만료**된다.
...
```
[시점 0초] 캐시 세팅 (TTL=600s)
    ↓
[시점 600초] 인기 Key 3개의 캐시가 동시에 만료
    ↓
[시점 600초 ~ 601초]
  서버 1: "캐시 없네? → DB 조회"
  서버 2: "캐시 없네? → DB 조회"
  서버 3: "캐시 없네? → DB 조회"
  ...
  서버 8 × 수백 동시 요청 = DB에 수천 건 동시 쿼리
    ↓
[시점 601초 ~ 630초] DB 과부하 → 응답 지연 → 타임아웃
    ↓
[시점 ~630초] 캐시가 다시 채워지며 자동 복구
```
...
base_ttl = 600
jitter = random.randint(0, 120)  # 0~120초 랜덤
redis.set(key, value, EX=base_ttl + jitter)

# 결과: 캐시 만료가 600초~720초 사이에 분산
```
...
def get_metadata(key):
    ...
    if redis.set(lock_key, "1", NX=True, EX=10):  # 락 획득 시도
        try:
            # 락을 잡은 스레드만 DB 조회
            value = db.query(f"SELECT * FROM metadata WHERE meta_key = %s", key)
            redis.set(key, value, EX=600 + random.randint(0, 120))
            return value
        finally:
        ...
        time.sleep(0.1)
        return redis.get(key) or get_metadata(key)  # 재시도
```
...
def set_with_logical_expiry(key, value, logical_ttl=600):
    data = json.dumps({
        "value": value,
...
```python
# 별도 스케줄러가 주기적으로 실행
HOT_KEYS = ["popular-banner-config", "main-page-layout", "event-popup-settings"]

def pre_warm_hot_keys():
    for key in HOT_KEYS:
        remaining_ttl = redis.ttl(key)
        if remaining_ttl < 60:  # 만료 1분 전이면
            value = db.query(f"SELECT * FROM metadata WHERE meta_key = %s", key)
            redis.set(key, value, EX=600 + random.randint(0, 120))
```
...
| 지표 | Before | After |
|------|--------|-------|
| 주기적 DB QPS 급증 | 10~100배 | 발생하지 않음 |
| 최대 응답 시간 | 5초+ | 200ms 이내 |
| 타임아웃 실패율 | 주기적 발생 | 0% |
...
락 대기 시간(~100ms)은 DB 과부하로 인한 5초+ 지연보다 훨씬 짧다. 또한 락은 **캐시 미스가 발생한 순간에만** 작동하므로, 평소에는 아무런 오버헤드가 없다.

### Q: 논리적 만료에서 stale 데이터를 반환하는 게 괜찮은가?

서비스 특성에 따라 다르다. 메타데이터(배너 설정, 페이지 레이아웃 등)는 수초 정도의 지연이 허용되는 경우가 많다. 결제 금액이나 재고 수량 같은 데이터에는 이 전략을 쓰면 안 된다.
